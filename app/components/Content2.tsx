...
const Content2 = ({formData, setFormData, isChecked, setIsChecked}: Content2Props) => {
  ...
  return (
    <div className="content2 bg-white mt-24 md:mt-0 fixed md:static w-11/12 rounded-xl px-6 xl:py-8 md:pt-4 md:py-0 py-6 md:col-start-2 md:w-full md:flex md:flex-col">
        <h1 className="text-2xl md:text-3xl font-bold py-3 xl:text-4xl">Select your plan</h1>
        <h2 className="text-lg md:text-xl opacity-40 font-bold md:leading-8 xl:text-2xl">You have the option of monthly or yearly billing.</h2>
        <fieldset className="grid grid-cols-1 grid-rows-3 md:flex md:items-stretch md:justify-between gap-4 my-4 md:my-6">
        ...
        <label className="flex justify-center items-center cursor-pointer gap-6 bg-slate-100 p-3 md:p-4 rounded-lg">
          {isChecked ? <span className="font-bold md:text-lg opacity-50">Monthly</span> : <span className="font-bold md:text-lg">Monthly</span>}
